...
import { useGalleryInfoStore } from '@/stores/modal';
...
const GalleryItem = ({
  ...
  const openModal = useGalleryInfoStore((state) => state.openModal);
  ...
  const onHandleNavigate = () => {
    ...
    openModal({ galleryId, hasEnded, open: true });
  };
  ...
  return (
    <S.Container onClick={onHandleNavigate}>
      ...
      <S.ContentBox>
        <Text typography="t6" color="white" bold="semibold" className="description">
          {formattedHashtags}
        </Text>
      </S.ContentBox>
      {hasEnded && <S.Overlay />}
